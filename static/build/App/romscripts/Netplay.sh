...
if [ "$romdirname" == "GB" ] || [ "$romdirname" == "GBC" ]; then
	...
	EasyNetplayPokemonScriptClient="$sysdir/script/netplay/easy-netplay_client_pokemon.sh"
fi
if [ "$romdirname" == "GBA" ]; then
	...
	EasyNetplayPokemonScriptClient="$sysdir/script/netplay/easy-netplay_client_pokemon_GBA.sh"
fi
...
if [ $retcode -eq 0 ]; then
	...
	if [ $retcode -eq 0 ]; then
		...
		/bin/sh "$EasyNetplayPokemonScriptHost" "$1"
	...
	if [ $retcode -eq 0 ]; then
		...
		/bin/sh "$EasyNetplayPokemonScriptClient" "$1"
	elif [ $retcode -eq 255 ]; then
		exit
	fi
