...
backup_and_send_save() {
    ...
    mkdir -p "/mnt/SDCARD/Saves/CurrentProfile/saves/gpSP"
    save_gambatte="/mnt/SDCARD/Saves/CurrentProfile/saves/Gambatte/$client_rom_filename_NoExt.srm"
    save_tgbdual="/mnt/SDCARD/Saves/CurrentProfile/saves/gpSP/$client_rom_filename_NoExt.srm"
...
start_retroarch() {
    ...
    HOME=/mnt/SDCARD/RetroArch ./retroarch --appendconfig=./.retroarch/easynetplay_override.cfg -C $hostip -L "$core" "$client_rom" "$rom"
...
wait_for_save_return() {
    ...
    if [ $cp_exit_status -eq 0 ]; then
        if [ $SaveFromGambatte -eq 1 ]; then
            ...
            mv -f "/mnt/SDCARD/Saves/CurrentProfile/states/gpSP/$client_rom_filename_NoExt.state.auto" "/mnt/SDCARD/Saves/CurrentProfile/states/gpSP/$client_rom_filename_NoExt.state.auto_$CurDate"
        fi
