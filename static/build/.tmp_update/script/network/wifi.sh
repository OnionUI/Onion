...
get_wpa_state() {
    $WPA status | grep '^wpa_state=' | cut -d'=' -f2
}

get_connected_wifi() {
    wpa_status=$(wpa_cli status)
    wpa_state=$(echo "$wpa_status" | grep '^wpa_state=' | cut -d'=' -f2)
    ssid=$(echo "$wpa_status" | awk '/^ssid=/{sub("ssid=","");print;exit}')
...
delete_known_wifi() {
    if [ -n "$1" ]; then
        if [ "$1" -eq "$1" ] 2>/dev/null; then
            ...
            log "Deleting known Wi-Fi network: $1"
            $WPA remove_network "$1"
            $WPA save_config
        else
            id=$(is_known "$1")
            if [ -n "$id" ]; then
                log "Deleting known Wi-Fi network: $1 (ID:$id)"
                $WPA remove_network "$id"
                $WPA save_config
            else
                log "Error: Unknown Wi-Fi network: $1"
                exit 1
            fi
        fi
