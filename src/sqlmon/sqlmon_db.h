...
void sql_shutdown()
{
    ...
    sqlite3_finalize(total_play_time);
    sqlite3_finalize(find_all);
    sqlite3_finalize(insert_rom);
    sqlite3_finalize(rom_id);
    sqlite3_finalize(update_rom);
    sqlite3_finalize(orphan_rom);
    sqlite3_finalize(rom_by_path);
    sqlite3_finalize(rom_playtime);
    sqlite3_finalize(active_closed_activity);
    sqlite3_finalize(activity_start);
    sqlite3_finalize(activity_stop);
    sqlite3_finalize(activity_stop_all);
    sqlite3_finalize(delete_null_entry);
    sqlite3_finalize(get_file_path);
...
int play_activity_db_execute(sqlite3_stmt *sql)
{
    ...
    int rc = sqlite3_exec(play_activity_db_writer, "BEGIN IMMEDIATE;", NULL, NULL, NULL);
    if (rc != SQLITE_OK)
        return rc;
    rc = sqlite3_step(sql);
    if (rc != SQLITE_DONE) {
