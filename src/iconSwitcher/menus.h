...
int _add_icon_alts(const char *pack_dir, const char *pack_name, const char *icon_prefix, List *list, void (*action)(void *))
{
	DIR *dp;
	struct dirent *ep;
	...
	if ((dp = opendir(pack_dir)) != NULL) {
        while ((ep = readdir(dp))) {
			...
			if (strncmp(icon_prefix, ep->d_name, strlen(icon_prefix)) != 0 || ep->d_name[strlen(icon_prefix)] != '-') continue;

            snprintf(preview_path, STR_MAX * 2, "%s/%s", pack_dir, ep->d_name);

			strncpy(icon_name, ep->d_name, STR_MAX - 1);
			snprintf(alt_name, STR_MAX - 1, "%s - %s", pack_name, file_removeExtension(str_split(icon_name, "-")));
	...
	if ((dp = opendir(path)) != NULL) {
        while ((ep = readdir(dp))) {
			...
			if (required_icon != NULL) {
            	...
            	snprintf(preview_path, STR_MAX * 2 + 31, "%s/preview.png", icon_pack_path);

				if (!is_file(preview_path))
					snprintf(preview_path, STR_MAX * 2 + 31, "%s/gba.png", icon_pack_path);
			}
            ...
            if (is_dir(icon_pack_path)) {
				...
				if (required_icon != NULL)
					count += _add_icon_alts(icon_pack_path, icon_pack_name, required_icon, list, action);
            }
        }
        closedir(dp);
	}
...
void _action_apply_icon_pack(void *_item)
{
    ...
    theme_renderDialog(screen, item->label, "Do you want to apply\nthis icon pack?", true);
...
bool _add_config_icon(const char *path, const char *name, const char *config_path, List *list, void (*action)(void *))
{
	char label[STR_MAX];
	char icon_name[56];
	...
	if (icon_path[0] != '/')
		snprintf(preview_path, STR_MAX*2+32 - 1, "%s/%s/%s", path, name, icon_path);
	else
...
void menu_console_icons(void *_)
{
	if (!_menu_console_icons._created) {
		...
		_add_config_icons(CONFIG_EMU_PATH, &_menu_console_icons, menu_change_console_icon);
		_add_config_icon(CONFIG_EMU_PATH, "SEARCH", SEARCH_CONFIG_SRC, &_menu_console_icons, menu_change_console_icon);
