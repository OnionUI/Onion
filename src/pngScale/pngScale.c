...
//
void GFX_BlitSurface(MI_PHY srcPa, void* srcVa, uint32_t sw, uint32_t sh, MI_PHY dstPa, void *dstVa, uint32_t dw, uint32_t dh) {
...
int main(int argc , char* argv[]) {
	...
	uint8_t *src8;
	uint32_t *src, *dst, pix, x, y, sw, sh, dw, dh, ss = 0, ds = 0, mw = 250, mh = 360;

	// Read commandline and open src
	if (argc < 3) goto usage;
	if (argc > 3) mw = atoi(argv[3]);
	if (argc > 4) mh = atoi(argv[4]);
	fp = fopen(argv[1], "rb");
	if ((!fp)||(!mw)||(!mh)) goto usage;

	// Read png header and calc size
	if (fread(sig_bytes, sizeof(sig_bytes), 1, fp) != 1) ERROR("png format error");
	if (png_sig_cmp(sig_bytes, 0, sizeof(sig_bytes))) ERROR("png format error");
	...
	ss = ALIGN4K(sw * sh * 4);
	if ( (!sw)||(!sh)||(!ss)||(!ch) ) ERROR("png format error");

	// Initialize MI_lib and allocate src mem
	MI_SYS_Init();
	MI_GFX_Open();
	MI_SYS_MMA_Alloc(NULL, ss, &srcPa); MI_SYS_Mmap(srcPa, ss, &srcVa, TRUE);
	...
	switch (ch) {
		case 1:
			for (y=0; y<sh; y++) {
				src8 = rows[y];
				for (x=0; x<sw; x++, src8++) {
					*dst++ = 0xFF000000 | (src8[0]<<16) | (src8[0]<<8) | src8[0];
				}
			}
			break;
		case 2:
			for (y=0; y<sh; y++) {
				...
				for (x=0; x<sw; x++, src8 += 2) {
					*dst++ = (src8[1]<<24) | (src8[0]<<16) | (src8[0]<<8) | src8[0];
				}
			}
			break;
		case 3:
		...
		case 4:
			for (y=0; y<sh; y++) {
				...
				for (x=0; x<sw; x++) {
					pix = *src++;
					*dst++ = (pix & 0xFF00FF00) | (pix & 0x00FF0000)>>16 | (pix & 0x000000FF)<<16;
				}
			}
			break;
	}
	...
	// Calculate dst size
	dw = mw; dh = sh * dw / sw;
	if (dh > mh) { dh = mh; dw = sw * dh / sh; }
	ds = ALIGN4K(dw * dh * 4);

	// Allocate dst png mem and scale
	MI_SYS_MMA_Alloc(NULL, ds, &dstPa); MI_SYS_Mmap(dstPa, ds, &dstVa, TRUE);
	GFX_BlitSurface(srcPa, srcVa, sw, sh, dstPa, dstVa, dw, dh);
	MI_SYS_Munmap(srcVa, ss); MI_SYS_MMA_Free(srcPa); srcVa = NULL; srcPa = 0;
	printf("png scaling : w:%d h:%d -> w:%d h:%d\n", sw, sh, dw, dh);

	// Create png
	fp = fopen(argv[2], "wb");
	if (!fp) ERROR("png write error");
	...
	// Quit
	MI_SYS_Munmap(dstVa, ds); MI_SYS_MMA_Free(dstPa);
	MI_GFX_Close();
	MI_SYS_Exit();
...
usage:
	printf("usage: %s src.png dst.png [max_width:def=250] [max_height:def=360]\n",argv[0]);
error:
	if (srcVa) MI_SYS_Munmap(srcVa, ss);
	if (dstVa) MI_SYS_Munmap(dstVa, ds);
	if (srcPa) MI_SYS_MMA_Free(srcPa);
	if (dstPa) MI_SYS_MMA_Free(dstPa);
	return 1;
}
