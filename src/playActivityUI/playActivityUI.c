...
static PlayActivitiesUI *play_activities;
...
void free_resources(void)
{
    ...
    free_play_activities_ui(play_activities);
}
...
void renderPage(int current_page)
{
    ...
    for (int row = 0; row < 4; row++) {
        ...
        if (show_raw_names)
            strncpy(rom_name, entry->rom_name, STR_MAX - 1);
        else
            file_cleanName(rom_name, entry->rom_name);
        renderText(rom_name, includeCJK(rom_name) ? fontCJKRomName25 : font30, color_white, &(SDL_Rect){num_width + 100, 75 + 90 * row, 400, 40});

        snprintf(total, 25, "%s", entry->play_time_total);
        snprintf(average, 25, "%s", entry->play_time_average);
        snprintf(plays, 25, "%s", entry->play_count);
