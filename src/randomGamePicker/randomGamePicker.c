...
bool readDatabase(const char *cache_path, const char *table_name)
{
    ...
    const char *sql = sqlite3_mprintf(
        "SELECT * FROM %q WHERE type=0 ORDER BY RANDOM() LIMIT 1", table_name);
    ...
    while (sqlite3_step(res) == SQLITE_ROW) {
        ...
        cJSON_AddItemToObject(
            games, "rompath",
            cJSON_CreateString((const char *)sqlite3_column_text(res, 2)));
...
void readConfigFile(const char *emuname)
{
    ...
    if (exists(config_path)) {
        ...
        if (strncmp(config.rompath, "../../App/", 10) != 0) {
            ...
            snprintf(cache_path, STR_MAX * 3 - 1, "%s/%s/%s/%s_cache2.db",
                     "/mnt/SDCARD/Emu", emuname, config.rompath,
                     basename(config.rompath));
...
int main(int argc, char *argv[])
{
    ...
    snprintf(cmd_to_run, STR_MAX * 3, "%s \"%s\" \"%s\"",
             "LD_PRELOAD=/mnt/SDCARD/miyoo/app/../lib/libpadsp.so", emu_path,
             all_games[random_number].rompath);
