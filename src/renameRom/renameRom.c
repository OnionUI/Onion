...
void _path(char *dest, const char *dir_path, const char *file_name, const char *file_ext)
{
    snprintf(dest, STR_MAX * 3 - 1, "%s/%s.%s", dir_path, file_name, file_ext);
}

bool renameFile(const char *dir_path, const char *file_ext, const char *old_name, const char *new_name)
{
...
bool renameCache(const char *cache_path, const char *rom_dirname, const char *rom_path, const char *new_rompath, const char *new_imgpath, const char *new_name)
{
    sqlite3 *db;
    sqlite3_stmt *res;
    ...
    const char *sql = sqlite3_mprintf("UPDATE %q_roms SET path = %Q, imgpath = %Q, disp = %Q WHERE path = %Q", rom_dirname, new_rompath, new_imgpath, new_name, rom_path);
...
int main(int argc, char *argv[])
{
    ...
    snprintf(cache_path, STR_MAX * 3 - 1, "%s/%s/%s_cache2.db", emupath, config.rompath, basename(config.rompath));
    ...
    renameCache(cache_path, basename(config.rompath), rompath, new_rompath, new_imgpath, new_name);
