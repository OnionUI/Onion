...
void __list_ensureVisible(List *list, int direction, int items_left)
{
    if (list->items[list->active_pos].disabled) {
        list->active_pos = _list_modulo(list->active_pos + direction, list->item_count);
        if (items_left > 0) {
            __list_ensureVisible(list, direction, items_left - 1);
        }
    }
}

void _list_ensureVisible(List *list, int direction)
{
    __list_ensureVisible(list, direction, list->item_count);
}
...
void __list_scroll(List *list, int pos)
{
...
bool list_keyUp(List *list, bool key_repeat)
{
    ...
    if (_list_did_wraparound(old_pos, list->active_pos, -1)) {
        if (list->scroll_pos > 0) {
            __list_scroll(list, list->scroll_pos - 1);
            list->active_pos = old_pos;
        }
        else {
            __list_scroll(list, list->item_count - 1);
        }
    }
    else {
        _list_scroll(list);
...
bool list_keyDown(List *list, bool key_repeat)
{
    ...
    if (_list_did_wraparound(old_pos, list->active_pos, 1)) {
        if (list->scroll_pos < list->item_count - list->scroll_height) {
            __list_scroll(list, list->scroll_pos + list->scroll_height);
            list->active_pos = old_pos;
        }
        else {
            __list_scroll(list, 0);
        }
    }
    else {
        _list_scroll(list);
