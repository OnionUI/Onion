...
int main(int argc , char* argv[]) {
	...
	if (argc > 2) mw = atoi(argv[2]);
	if (argc > 3) mh = atoi(argv[3]);
	fp = fopen(argv[1], "rb");
	...
	sw = jpeg.output_width; sh = jpeg.output_height; ss = ALIGN4K(sw * sh * 4);
	if ((!sw)||(!sh)||(!ss)||(jpeg.out_color_components != 3)) { fprintf(stderr, "jpg format error\n"); goto error; }

	// Initialize MI_lib and allocate jpg mem
	MI_SYS_Init();
	MI_GFX_Open();
	MI_SYS_MMA_Alloc(NULL, ss, &jpgPa); MI_SYS_Mmap(jpgPa, ss, &jpgVa, TRUE);
	...
	MI_SYS_MMA_Alloc(NULL, ds, &pngPa); MI_SYS_Mmap(pngPa, ds, &pngVa, TRUE);
	GFX_BlitSurface(jpgPa, jpgVa, sw, sh, pngPa, pngVa, dw, dh);
	MI_SYS_Munmap(jpgVa, ss); MI_SYS_MMA_Free(jpgPa); jpgVa = NULL; jpgPa = 0;
	printf("sw:%d sh:%d dw:%d dh:%d\n", sw, sh, dw, dh);

	// Create png
	strcpy(filename, argv[1]);
	ptr = strrchr(filename, '.'); if (ptr) *ptr = 0;
	strcat(filename, ".png");
	fp = fopen(filename, "wb");
	if (!fp) { fprintf(stderr, "png write error\n"); goto error; }
	...
	for (y=0; y<dh; y++) {
		for (x=0; x<dw; x++){
			pix = *src++;
			dst[x] = 0xFF000000 | (pix & 0x0000FF00) | (pix & 0x00FF0000)>>16 | (pix & 0x000000FF)<<16;
		}
	...
	// Quit
	MI_SYS_Munmap(pngVa, ds); MI_SYS_MMA_Free(pngPa);
	MI_GFX_Close();
	MI_SYS_Exit();
...
error:
	if (jpgVa) MI_SYS_Munmap(jpgVa, ss);
	if (pngVa) MI_SYS_Munmap(pngVa, ds);
	if (jpgPa) MI_SYS_MMA_Free(jpgPa);
	if (pngPa) MI_SYS_MMA_Free(pngPa);
	return 1;
}
