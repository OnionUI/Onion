...
for file in $(eval "find /mnt/SDCARD/Roms/$CurrentSystem -maxdepth 2 -type f \
	...
	if [ -f "/mnt/SDCARD/Roms/$CurrentSystem/Imgs/$romNameNoExtension.png" ]; then
        ...
        # TODOO provide from JSON and a default / configurable option
        fallbackpreference="wo"
        json_regions="fr eu us" #$(cat /path/to/regions.json)
        
        #calculate with final fallbackoption
        priotitizedregions=$(find_region "$json_regions" $fallbackpreference)

        calculated_regional_preferences="($priotitizedregions)"

        # Initialize URL aand Iterate over list of region codes
        qwaurl=""
        for region in "${calculated_regional_preferences[@]}"; do
        ...
        # If no url was found after checking all regions, print an error message
        if [ -z "$url" ]; then
